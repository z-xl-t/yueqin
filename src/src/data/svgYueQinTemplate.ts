...
for(let i=0; i< y.length; ++i) {
  svgYueQinTemplate.allPoint.push([] as SvgTextAttrType[])
  for (let j=0; j < x.length; ++j) {
    svgYueQinTemplate.allPoint[i].push({x: x[j], y: y[i], content: `${i}${j}`})
  }
}
let newItem = [...svgYueQinTemplate.allPoint]
newItem = newItem.slice(2,newItem.length - 1)
newItem.forEach(item => {
  const subItem = [...item];
  svgYueQinTemplate.yinfuPoint.push(subItem.slice(2,subItem.length - 1))
});
...
for(let i=0; i<xx.length; ++i) {
  const x = xx[i] - 30
  const y = yy[0]
  svgYueQinTemplate.xianName.push({x, y, content: `第 ${xianNum - i} 弦`})
  const x1 = xx[i]
  const y1 = yy[1]
  const x2 = xx[i]
  const y2 = yy[2]
  svgYueQinTemplate.xian.push({x1, y1, x2,y2, stroke})
}
...
for (let i=0; i<yyy.length; ++i) {
  const x = xxx[0]
  const y = yyy[i]
  svgYueQinTemplate.pingName.push( {x,y, content: `第 ${pingNum - i} 品`})
  const x1 = xxx[1]
  const y1 = yyy[i]
  const x2 = xxx[2]
  const y2 = yyy[i]
  svgYueQinTemplate.ping.push({x1, y1,x2,y2, stroke})
}
export default svgYueQinTemplate;
