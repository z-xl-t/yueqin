...
import { jianPus } from '@/data/jianPus'
...
const equalTemperament = ref(equalTemperamentAll.value[0])
const banyinFlag = ref(false)
...
<template>
  <div>
    <q-select
      style="width: 200px"
      filled
      v-model="equalTemperament"
      option-label="comment"
      :options="equalTemperamentAll"
      label="调号"
    />
    <!-- <q-toggle label="是否隐藏半音" v-model="banyinFlag" color="primary" keep-color /> -->
  </div>
